...
jQuery.fn.dataTable.ext.type.order['file-size-desc'] = function ( data ) {
    if (data === null && data === '') {
        return 0;
    }

    // var matches = (data.substring(data.length - 2, data.length).toLowerCase() == 'mb' ?
    //         1000 : (data.substring(data.length - 2, data.length).toLowerCase() == 'gb' ? 1000000 : 1));
